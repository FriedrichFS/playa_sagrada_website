...
def sign_up():
    if request.method == "POST":
        ...
        if user:
            ...
            if len(email) < 4:
                flash("Email muss mindestens 4 Buchstaben beinhalten.",
                      category="error")
            elif len(first_name) < 2:
                flash("Vorname muss mindestens 2 Buchstaben beinhalten.",
                      category="error")
            elif len(second_name) < 2:
                flash("Nachname muss mindestens 4 Buchstaben beinhalten.",
                      category="error")
            elif len(password1) < 4:
                flash(
                    "Passwort muss mindesten 4 Buchstaben oder Zeichen enthalten.", category="error")
                ...
                flash(
                    "Bitte gebe einen gültige Wert ein! (Geschäftsführung / Vollzeit / Teilzeit / Werkstudent / Mini-Job", category="error")
            else:
                new_user = User(email=email.lower(), first_name=first_name, second_name=second_name, password=generate_password_hash(
                    password1, method="sha256"), user_stat=user_stat, user_dep=user_dep)
...
def enter_times():
    if request.method == "POST":
        ...
        worked_times = Arbeit.query.order_by(
            Arbeit.date).filter_by(date=date).all()
        new_time = Arbeit(date=date, von=von, bis=bis,
                          user_id=current_user.id, length=length, break_user=0)
        ...
        if not date:
            ...
            for time in worked_times:
                ...
                amount = amount + (end_hours - start_hours) * \
            ...
            if not time_delta_status:
                flash(
                    "Dein Speicherungsversuch überschneidet sich mit einem bereits vorhandenen Eintrag!", category="error")
                ...
                new_arbeitszeit = Arbeit(date=date, von=von, bis=bis, user_id=current_user.id, length=round(
                    calc_complete_time(length, break_time_converted), 2), break_user=break_time_converted)
                db.session.add(new_arbeitszeit)
                db.session.commit()
                flash("Deine Arbeitszeit wurde für den " + date + " für den Zeitraum " +
                      von + " Uhr bis " + bis + " Uhr erfolgreich gespeichert", category="success")
...
def edit_times():
    if request.method == "POST":
        ...
        if not date:
            ...
            data.delete(note)
            db.session.commit()
            flash("Arbeitszeiten vom " + str(date) +
                  " wurden erfolgreich gelöscht!", category="success")
            return redirect(url_for("views.home"))
        else:
            flash("Das ausgewählte Datum ist noch nicht eingespeichert!",
                  category="error")
...
def export():
    ...
    if request.method == "POST":
        ...
        january_dict = {"Datum": [], "Von": [],
                        "Bis": [], "Pause": [], "Gesamt": []}
        february_dict = {"Datum": [], "Von": [],
                         "Bis": [], "Pause": [], "Gesamt": []}
        march_dict = {"Datum": [], "Von": [],
                      "Bis": [], "Pause": [], "Gesamt": []}
        april_dict = {"Datum": [], "Von": [],
                      "Bis": [], "Pause": [], "Gesamt": []}
        may_dict = {"Datum": [], "Von": [],
                    "Bis": [], "Pause": [], "Gesamt": []}
        june_dict = {"Datum": [], "Von": [],
        ...
        july_dict = {"Datum": [], "Von": [],
                     "Bis": [], "Pause": [], "Gesamt": []}
        august_dict = {"Datum": [], "Von": [],
                       "Bis": [], "Pause": [], "Gesamt": []}
        september_dict = {"Datum": [], "Von": [],
                          "Bis": [], "Pause": [], "Gesamt": []}
        october_dict = {"Datum": [], "Von": [],
                        "Bis": [], "Pause": [], "Gesamt": []}
        november_dict = {"Datum": [], "Von": [],
                         "Bis": [], "Pause": [], "Gesamt": []}
        december_dict = {"Datum": [], "Von": [],
                         "Bis": [], "Pause": [], "Gesamt": []}

        # e lists for  month contai, from, until length worked on that day
        export_queryd_user = User.query.filter_by(
            first_name=export_user).first()
        ...
        flash("Es wurden erfolgreich die Daten von " + str(export_user) +
              " des Monats " + str(month) + " exportiert!", category="success")
        ...
        if not old_password:
            ...
            if check_password_hash(current_user.password, old_password):
                if new_password_1 == new_password_2:
                    ...
                    change_user.password = generate_password_hash(
                        new_password_1, method="sha256")
                    db.session.commit()
                    flash(
                        f"Passwort erfolgreich zurück gesetzt! - Logge {current_user.first_name} aus",)
                    logout()
...
def reset_request():
    if request.method == "POST":
        ...
        if not user:
            flash(
                "Es wurde kein Account mit dieser E-Mail Adresse gefunden", category="error")
        else:
...
def generate_verification_code():
    ...
    if request.method == 'POST':
        ...
        if check_verification_token(to_email, verification_code):
            ...
            flash("Falscher Verifikationscode. Bitte erneut versuchen! ",
                  category="error")
...
def new_password():
    if request.method == "POST":
        ...
        if not new_password_1:
            ...
            if new_password_1 == new_password_2:
                change_user = User.query.filter_by(
                    email=session["to_email"]).first()
                change_user.password = generate_password_hash(
                    new_password_1, method="sha256")
                db.session.commit()

                flash("Passwort geändert. Bitte logge dich erneut ein!",
                      category="success")
                return redirect(url_for("auth.login"))
...
def coworker_times():
    ...
    if request.method == "POST":
        ...
        user_to_display = User.query.filter_by(
            first_name=user_first_name, second_name=user_second_name).first()
